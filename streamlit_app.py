...
def full_cross_check(df_bom, df_xy):
    ...
    for _, row in df_bom.iterrows():
        ...
        mfrs = get_values(row, ["Hãng", "Manufacturer"])
    ...
    for pos, info in all_bom_pos.items():
        ...
        if match.empty:
            ...
            # Check Hãng
            xy_mfrs = get_values(xy_row, ["Hãng", "Manufacturer"])
...
df_bom = None
if f_bom:
    # Cho phép chọn Sheet vì file LO có sheet changeLog ở đầu
...
if f_xys:
    list_xy = []
    for f in f_xys:
        # Tự động đọc sheet đầu tiên của file XY
        t = pd.read_excel(f)
        t["File Nguồn"] = f.name
        list_xy.append(t)
